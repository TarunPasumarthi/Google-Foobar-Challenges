...
(string) “243**93*5++”
*/
public class rusty_calc {
	
	public static void main(String [] args){
		System.out.println(getLargestExpression("2*4*3+9*3+5"));
	}
	
	public static String getLargestExpression(String expression){
		String result="";
		...
		for(int i=0; i<expression.length();i++){
			...
			if(symbol=='+'){
				if(lastOperator=='*'){
					result=result+multiplicationString;
		...
		result=result+multiplicationString+additionString;
		return result;
	}
}
