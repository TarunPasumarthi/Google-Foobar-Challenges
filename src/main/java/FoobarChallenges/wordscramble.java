...
public class wordscramble {
    ...
    public static String answer(String chunk, String word) {
    	if(word.length()==1){
    		return chunk.replaceAll(word, "");
    	}
    	pq= new PriorityQueue<String>();
    	addPossibleStrings(chunk,word);
    	return pq.peek();
    }
	public static void addPossibleStrings(String chunk, String word){
		ArrayList<Integer> indexes= wordOccurances(chunk,word);
		if(indexes.isEmpty()){
			pq.add(chunk);
			return;
		}
		for(int i=0;i<indexes.size();i++){
			...
			String nextChunk= chunk.substring(0,index)+chunk.substring(index+word.length());
			addPossibleStrings(nextChunk,word);
		}
	}
	public static ArrayList<Integer> wordOccurances(String chunk, String word){
		ArrayList<Integer> indexes= new ArrayList<Integer>();
		int indexCount=0;
		while(true){
			int relative_index= chunk.indexOf(word);
			if(relative_index==-1){
				break;
			}
			chunk=chunk.substring(relative_index+1);
			indexes.add(indexCount+relative_index);
			indexCount=indexCount+relative_index+1;
		}
		return indexes;
	}
}
